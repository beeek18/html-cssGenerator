...
const totalResult = () => {
  total.innerHTML = `border-radius:${rangeTl.value}px ${rangeTr.value}px ${rangeBr.value}px ${rangeBl.value}px`
}
...
function copy() {
  navigator.clipboard.writeText(` border-radius:${rangeTl.value}px ${rangeTr.value}px ${rangeBr.value}px ${rangeBl.value}px`)
}